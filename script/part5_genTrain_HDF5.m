...
folderName = 'HDF5train_pixelMean';
if exist(folderName, 'dir')
    system( ['rm -rf ./' folderName]);
end
mkdir(folderName);

filenamePath = 'trainHDF5List_pixelMean.txt'; fn = fopen(filenamePath, 'w'); fclose(fn);
imListFile = './trainList.txt';
...
%         im = single(im) - meanImg;
    ...
    %% put in HDF5
    filenameTMP = fullfile( folderName, sprintf('train_part%03d.h5', batchno ) );
